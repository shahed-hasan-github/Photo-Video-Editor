...
public class ScanMP3 extends SoundFile {
    ...
    public void readFile(File inputFile)
        ...
        try (FileInputStream stream = new FileInputStream(mInputFile)) {
            ...
            while (pos < mFileSize - 12) {
                ...
                if (mpgVersion == 1) {
                    // MPEG 1 Layer III
                    bitRate = bitratesMpeg1L3[(buffer[2] & 0xF0) >> 4];
                    sampleRate = sampleratesMpeg1L3[(buffer[2] & 0x0C) >> 2];
                } else {
                    // MPEG 2 Layer III
                    bitRate = bitratesMpeg2L3[(buffer[2] & 0xF0) >> 4];
                    sampleRate = sampleratesMpeg2L3[(buffer[2] & 0x0C) >> 2];
                }
    ...
    private static  int[] bitratesMpeg1L3 = {
            0,  32,  40,  48,  56,  64,  80,  96,
            112, 128, 160, 192, 224, 256, 320,  0 };
    private static  int[] bitratesMpeg2L3 = {
            0,   8,  16,  24,  32,  40,  48,  56,
            64,  80,  96, 112, 128, 144, 160, 0 };
    private static  int[] sampleratesMpeg1L3 = {
            44100, 48000, 32000, 0 };
    private static int[] sampleratesMpeg2L3 = {
            22050, 24000, 16000, 0 };
}
